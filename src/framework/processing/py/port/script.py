...
def retry_no_data_confirmation():
    text = props.Translatable(
        {
            "en": f"Unfortunately we could not detect any location information in your file. This could be because you have turned of google location tracking. You will still be able to receive your reward. Click 'Continue' and proceed. If you have selected the wrong file by accident, then click 'Try again'.",
            "nl": f"Het lijkt erop dat er geen locatiegegevens in uw pakketje zitten. Dat kan bijvoorbeeld omdat u dit ooit hebt uitgezet. Ook met een leeg pakketje komt u in aanmerking voor de vergoeding. Druk op 'Verder' om door te gaan. Als u per ongeluk het verkeerde bestand hebt gekozen, druk dan op 'Probeer opnieuw'.",
        }
    )
    ok = props.Translatable({"en": "Try again", "nl": "Probeer opnieuw"})
    cancel = props.Translatable({"en": "Continue", "nl": "Verder"})
...
def prompt_file():
    description = props.Translatable(
        {
            "en": f"Click 'Choose file' to choose the file that you received from Google. If you click 'Continue', the data that is required for research is extracted from your file. This may take some time. Thank you for your patience.",
            "nl": f"Klik op ‘Kies bestand’ om het bestand dat u ontvangen hebt van Google te kiezen. Als u op 'Verder' klikt worden de gegevens die nodig zijn voor het onderzoek uit uw bestand gehaald. Dit kan soms even duren. Een moment geduld a.u.b.",
        }
    )
